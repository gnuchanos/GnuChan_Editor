...
class USERPREF_MT_addons_online_resources(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator(
                ...
                ).url = "http://wiki.gnuchanos.org/index.php/Extensions:2.6/Py/Scripts"

        layout.separator()

        layout.operator(
                "wm.url_open", text="How to share your add-on", icon='URL',
                ).url = "http://wiki.gnuchanos.org/index.php/Dev:Py/Sharing"
        layout.operator(
                "wm.url_open", text="Add-on Guidelines", icon='URL',
                ).url = "http://wiki.gnuchanos.org/index.php/Dev:2.5/Py/Scripts/Guidelines/Addons"
        layout.operator(
                ...
                if info["show_expanded"]:
                    ...
                    if tot_row:
                        ...
                        split.operator("wm.url_open", text="Report a Bug", icon='URL').url = info.get(
                                "tracker_url",
                                "https://developer.gnuchanos.org/maniphest/task/edit/form/2")
                        if user_addon:
