...
class WM_MT_splash(Menu):
    ...
    def draw(self, context):
        ...
        # temporary Range Game Engine
        col1.operator("wm.read_homefile", text="New project", icon='NEW')
        ...
        if sys.platform[:3] == "win":
            col1.operator("wm.console_toggle", text="Open console", icon='CONSOLE')
        col1.label("Range Engine Network:")
        ...
        #bpy.types.TOPBAR_MT_file_new.draw_ex(col1, context, use_splash=True) # not now son...
        ...
        api_url = ("https://rangeengine.tech/api/" + (bpy.app.range_version_string[0] + bpy.app.range_version_string[2]) + "/html/index.html")
        ...
        if found_recent:
            ...
            col2.operator("wm.url_open", text="Range Engine - Website", icon='URL').url = "https://rangeengine.tech"
            col2.operator("wm.url_open", text="Range Engine - Python API", icon='FILE_SCRIPT').url = api_url
            col2.operator("wm.url_open", text="Support the Development", icon='PATREON').url = "https://www.patreon.com/rangeengine"
...
class WM_MT_splash_about(Menu):
    ...
    def draw(self, context):
        ...
        col.scale_y = 0.8
        col.label(text="Using Blender {version} + 3.0 partial adaptions".format(version=bpy.app.version_string), translate=False)
        col.separator(factor=2.5)
        col.label(text=("Date: %s %s") % (bpy.app.build_commit_date.decode('utf-8', 'replace'),
                                                bpy.app.build_commit_time.decode('utf-8', 'replace')), translate=False)
        col.label(text=("Hash: %s") % bpy.app.build_hash.decode('ascii'), translate=False)
        col.label(text=("Branch: %s") % bpy.app.build_branch.decode('utf-8', 'replace'), translate=False)

        col = split.column(align=True)
        
        col.emboss = 'PULLDOWN_MENU'
        col.operator("wm.url_open", text="License", icon='URL').url = "https://www.blender.org/about/license/"
        col.operator("wm.url_open", text="Range Engine - Website", icon='BLENDER').url = "https://rangeengine.tech"
