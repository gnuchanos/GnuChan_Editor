...
GHOST_TSuccess GHOST_ContextWGL::initializeDrawingContext()
{
	     ...
	     strcmp(renderer, "GDI Generic") == 0) && version[0] == '1' && version[2] == '1')
	{
		MessageBox(m_hWnd, "Your system does not use 3D hardware acceleration.\n"
		                   "Range Engine requires a graphics driver with OpenGL 2.1 support.\n\n"
		                   "This may be caused by:\n"
		                   "* A missing or faulty graphics driver installation.\n"
		                   "  Range needs a graphics card driver to work correctly.\n"
		                   "* Accessing Range through a remote connection.\n"
		                   "* Using Range through a virtual machine.\n\n"
		                   "The program will now close.",
		           "Range - Can't detect 3D hardware accelerated Driver!",
		           MB_OK | MB_ICONERROR);
		exit(0);
	}
	else if (version[0] < '2' || (version[0] == '2' && version[2] < '1')) {
		MessageBox(m_hWnd, "Range requires a graphics driver with OpenGL 2.1 support.\n\n"
		                   "The program will now close.",
		           "Range - Unsupported Graphics Driver!",
		           MB_OK | MB_ICONERROR);
		exit(0);
	}
