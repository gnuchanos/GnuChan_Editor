...
GHOST_TSuccess GHOST_WindowWin32::setClientBorderless()
{
...
GHOST_TWindowState GHOST_WindowWin32::getState() const
{
	...
	else if ((style & WS_POPUP) && !(style & WS_OVERLAPPEDWINDOW)) {
		state = GHOST_kWindowStateFullScreen; // Borderless windowed
	}
	else {
...
GHOST_TSuccess GHOST_WindowWin32::setState(GHOST_TWindowState state)
{
	...
	switch (state) {
		...
		case GHOST_kWindowStateFullScreen:
			...
			break;
		case GHOST_kWindowStateBorderless:
			style = GetWindowLong(m_hWnd, GWL_STYLE);
			exStyle = GetWindowLong(m_hWnd, GWL_EXSTYLE);

			::SetWindowLong(m_hWnd, GWL_STYLE, (style & ~(WS_OVERLAPPEDWINDOW)) | WS_POPUP);
			::SetWindowLong(m_hWnd, GWL_EXSTYLE, exStyle & ~(WS_EX_DLGMODALFRAME | WS_EX_CLIENTEDGE | WS_EX_STATICEDGE));
			break;
