...
AUD_API AUD_Handle *AUD_PlaybackManager_play(AUD_PlaybackManager* manager, AUD_Sound* sound, unsigned int catKey)
{
	assert(manager);
	assert(sound);

	return new AUD_Handle((*manager)->play(*sound, catKey));
}
