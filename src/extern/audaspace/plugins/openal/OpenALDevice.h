...
private:
	...
	class OpenALHandle : public IHandle, public I3DHandle, public IHandleEffect
	{
	private:
		...
		OpenALEffect *m_effect;

		/// Current effect type.
		aud::SoundEffectType m_effectType;

		/// Current filter type.
		aud::SoundFilterType m_filterType;

		/// To check the pattern of values, avoid problems.
		inline bool checkValidEffect(aud::SoundEffectType type) {
			if (!m_effect)
				return false;
		...
		/// For sound with an effect activated, as soon as the sound ends we need to give it time to finally delete the sound.
		bool m_toBeStopped;
	...
	public:
		...
		virtual OpenALEffect *getEffect();
		virtual void setEffect(int effectType, int filterType);
		virtual void removeEffect();
		virtual bool hasEffect();
		virtual int getEffectType();
		virtual int getFilterType();

		virtual bool setReverbDensity(float density);
		virtual bool setReverbDiffusion(float diffusion);
		virtual bool setReverbGain(float gain);
		virtual bool setReverbGainHF(float gain_hf);
		virtual bool setReverbDecayTime(float decay_time);
		virtual bool setReverbDecayHFRatio(float decay_hf_ratio);
		virtual bool setReverbReflectionsGain(float reflection_gain);
		virtual bool setReverbReflectionsDelay(float reflection_delay);
		virtual bool setReverbLateReverbGain(float late_reverb_gain);
		virtual bool setReverbLateReverbDelay(float late_reverb_delay);
		virtual bool setReverbAirAbsorptionGainHF(float air_absorption);
		virtual bool setReverbRoomRolloffFactor(float rolloff_factor);
		virtual bool setReverbDecayLimitHF(int decay_limit_hf);
		virtual bool setFilterGain(float gain);
		virtual bool setFilterGainLF(float gainLF);
		virtual bool setFilterGainHF(float gainHF);
	};
