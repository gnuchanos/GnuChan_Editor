                                  ...
                                  const int alphaBackground)
{
	...
	if (!borderlessWindow) {
		...
		window->setState(GHOST_kWindowStateBorderless);
		window->setClientBorderless();
	}
	window->setCursorVisibility(false);
...
#endif
	{
		...
		if (GHOST_ISystem::createSystem() == GHOST_kSuccess) {
			...
			// of scope before GHOST_ISystem::disposeSystem() is called.
			{
				...
				strcpy(filename, (std::string(BKE_appdir_program_dir()) + "\\main.range").c_str());
#else
				...
				do {
					...
					if (!bfd) {
						...
						if ((!fullScreenParFound) && (!windowParFound)) {
							...
							if ((scene->gm.playerflag & GAME_PLAYER_FULLSCREEN) && !(scene->gm.playerflag & GAME_PLAYER_BORDERLESS_WINDOW)) {
								fullScreen = true;
...
#endif
								{
									if (parentWindow != 0) {
										...
										window = startWindow(system, title, windowLeft, windowTop, windowWidth,
										                     windowHeight, stereoWindow, (scene->gm.playerflag & GAME_PLAYER_BORDERLESS_WINDOW), alphaBackground);
									}
								}
							}
