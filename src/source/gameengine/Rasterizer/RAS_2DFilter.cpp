...
static std::string predefinedUniformsName[RAS_2DFilter::MAX_PREDEFINED_UNIFORM_TYPE] = {
	...
	"ge_toneColorParams", // GE_TONEMAP_PARAMETERS_UNIFORM
	...
	"ge_ssaoGIirradiance", // GE_SSAO_GI_PARAMETERS_UNIFORM
	"ge_vignetteParams" // GE_VIGNETTE_PARAMETERS_UNIFORM
};
                                    ...
                                    RAS_OffScreen *colorofs, RAS_OffScreen *targetofs, const float (&sun_screen_pos)[2])
{
	...
	BindUniforms(canvas, sun_screen_pos);
...
void RAS_2DFilter::BindUniforms(RAS_ICanvas *canvas, const float (&sun_screen_pos)[2])
{
	if (m_predefinedUniforms[RENDERED_TEXTURE_UNIFORM] != -1) {
	...
	if (m_predefinedUniforms[GE_TONEMAP_COLOR_PARAMETERS_UNIFORM] != -1) {
		float params[2] = {m_buildInFilters.tonemap_saturation, m_buildInFilters.tonemap_temperature};
		SetUniformfv(m_predefinedUniforms[GE_TONEMAP_COLOR_PARAMETERS_UNIFORM], RAS_Uniform::UNI_FLOAT2, params, sizeof(float) * 2, 1);
	}
	...
	if (m_predefinedUniforms[GE_SSAO_PARAMETERS_UNIFORM] != -1) {
		float params[4] = {(float)m_buildInFilters.ssao_samples, m_buildInFilters.ssao_strength, 
							m_buildInFilters.ssao_distance, m_buildInFilters.ssao_attenuation};
		SetUniformfv(m_predefinedUniforms[GE_SSAO_PARAMETERS_UNIFORM], RAS_Uniform::UNI_FLOAT4, params, sizeof(float) * 4, 1);
	}

	if (m_predefinedUniforms[GE_SSAO_GI_PARAMETERS_UNIFORM] != -1) {
		float param[2] = {(float)m_buildInFilters.use_SSAO_GI, (float)m_buildInFilters.ssao_gi_irradiance};
		SetUniformfv(m_predefinedUniforms[GE_SSAO_GI_PARAMETERS_UNIFORM], RAS_Uniform::UNI_FLOAT2, param, sizeof(float) * 2, 1);
	}

	if (m_predefinedUniforms[GE_VIGNETTE_PARAMETERS_UNIFORM] != -1) {
		float params[2] = {m_buildInFilters.vignette_size, m_buildInFilters.vignette_radius};
		SetUniformfv(m_predefinedUniforms[GE_VIGNETTE_PARAMETERS_UNIFORM], RAS_Uniform::UNI_FLOAT2, params, sizeof(float) * 2, 1);
	}
