...
public:
	...
	int GetShadowBindCode(int bindcode_i);
	mt::mat4 GetViewMat();
	mt::mat4 GetWinMat();
	mt::mat4 GetShadowMatrix(int matrix_id); // matrix_id for CSM shadow
	const float GetShadowCSMProportion(int index);
	int GetShadowLayer();
	void CalculateWinMat(int csm_winmat_step);
	void BindShadowBuffer(RAS_ICanvas *canvas, KX_Camera *cam, mt::mat3x4& camtrans, int shadow_step);
