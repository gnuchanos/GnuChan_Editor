...
bool RAS_OpenGLLight::HasCSM()
{
	GPULamp *lamp;

	if ((lamp = GetGPULamp())) {
		return GPU_lamp_has_csm(lamp);
	}
	else {
		return false;
	}
...
int RAS_OpenGLLight::GetShadowBindCode(int bindcode_i)
{
	GPULamp *lamp;

	if ((lamp = GetGPULamp())) {
		return GPU_lamp_shadow_bind_code(lamp, bindcode_i);
	}
	return -1;
}
...
mt::mat4 RAS_OpenGLLight::GetShadowMatrix(int matrix_id)
{
	GPULamp *lamp;

	if ((lamp = GetGPULamp())) {
		return mt::mat4(GPU_lamp_dynpersmat(lamp, matrix_id));
	}

	return mt::mat4::Identity();
}

const float RAS_OpenGLLight::GetShadowCSMProportion(int index)
{
	GPULamp *lamp;

	if ((lamp = GetGPULamp())) {
		return GPU_lamp_get_csm_proportion(lamp, index);
	}
...
void RAS_OpenGLLight::CalculateWinMat(int csm_winmat_step)
{
	GPULamp *lamp;
	if ((lamp = GetGPULamp())) {
		GPU_lamp_calc_winmat(lamp, csm_winmat_step);
	}
}

void RAS_OpenGLLight::BindShadowBuffer(RAS_ICanvas *canvas, KX_Camera *cam, mt::mat3x4& camtrans, int shadow_step)
{
	GPULamp *lamp;
	...
	/* bind framebuffer */
	GPU_lamp_shadow_buffer_bind(lamp, shadow_step, viewmat, &winsize, winmat);
