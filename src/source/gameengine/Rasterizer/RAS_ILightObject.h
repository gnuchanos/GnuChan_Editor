...
public:
	...
	virtual bool HasCSM() = 0;
	virtual bool NeedShadowUpdate() = 0;
	virtual int GetShadowBindCode(int bindcode_i) = 0;
	virtual mt::mat4 GetShadowMatrix(int matrix_id) = 0;
	virtual mt::mat4 GetViewMat() = 0;
	virtual mt::mat4 GetWinMat() = 0;
	virtual const float GetShadowCSMProportion(int index) = 0;
	virtual int GetShadowLayer() = 0;
	virtual void CalculateWinMat(int csm_winmat_step) = 0;
	virtual void BindShadowBuffer(RAS_ICanvas *canvas, KX_Camera *cam, mt::mat3x4& camtrans, int shadow_step) = 0;
