...
void KX_PythonComponentManager::UpdateComponents()
{
	...
	const bool doProfiling = engine->GetDebugMode()->UseAdvancedProfiling();
	float timeProfiling;
	...
	for (KX_GameObject *gameobj : objects) {
		...
		if (doProfiling) {
			timeProfiling = (float)KX_GetActiveEngine()->GetTimeSecondClock();
		}
		...
		if (doProfiling) {
			// Add the time spent updating components.
			gameobj->SetDebugTimeProfiling(KX_GameObject::DebugProfilingType::TIME_COMPONENTS, timeProfiling);
		}
	}
}
