	...
	m_audio3d_frames(0)
{
	...
	if (settings.tonemap) {
		...
		filters.tonemap_saturation = settings.tonemap->saturation;
		filters.tonemap_temperature = settings.tonemap->temperature;
	}
	...
	if (settings.ssao) {
		...
		filters.ssao_lod = settings.ssao_lod;
		...
		bool useGI = (scene->scenefx_settings.scenefx_flag & SCENE_FX_FLAG_SSAO_GI) ? true : false;
		if (useGI) {
			filters.use_SSAO_GI = useGI;
			filters.ssao_gi_irradiance = settings.ssao->gi_irradiance;
		}
	}

	if (settings.vignette) {
		filters.useVignette = (scene->scenefx_settings.scenefx_flag & SCENE_FX_FLAG_VIGNETTE) ? true : false;
		filters.vignette_size = settings.vignette->size;
		filters.vignette_radius = settings.vignette->radius;
	}
	...
	if (ImGui::BeginPopup(popup.c_str())) {
		...
		switch (propval->GetValueType()) {
			...
			case VALUE_DATA_TYPE::VALUE_STRING_TYPE: {
				const std::string proptext = propval->GetText();
				char *oldprop = new char[propval->GetText().length() + 1];
				strcpy(oldprop, proptext.c_str());

				ImGui::Text("String: %s", proptext.c_str());

				if (ImGui::InputText(id.c_str(), oldprop, 1000)) {
...
void KX_Scene::UpdateAnimations(double curtime, bool restrict)
{
	...
	for (KX_GameObject *gameobj : m_animatedlist) {
		if (!gameobj->IsActionsSuspended()) {
			...
			if (eventManager) {
				for (KX_AnimationEvent *event : *eventManager->GetEvents()) {
					const std::vector<std::string> eventsToCall = event->GetEventsToCall();

					if (!eventsToCall.empty()) {
						for (std::string customArg : eventsToCall) {
							// We make sure it's thread safe. So we can make the call safely.
							PyGILState_STATE gilstate = PyGILState_Ensure();

							PyObject *args = PyTuple_New(2);
							PyObject *custom_arg = PyUnicode_FromString(customArg.c_str());
							PyTuple_SET_ITEM(args, 0, gameobj->GetProxy());
							PyTuple_SET_ITEM(args, 1, custom_arg);

							PyObject *function = event->GetPyEventFunction();
