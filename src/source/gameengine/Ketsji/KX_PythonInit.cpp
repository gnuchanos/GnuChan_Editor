...
static PyObject *gPySetFullScreen(PyObject *, PyObject *args)
{
	int type;
	if (!PyArg_ParseTuple(args, "i:type", &type)) {
		return nullptr;
	}

	KX_GetActiveEngine()->GetCanvas()->SetFullScreen(type);
	Py_RETURN_NONE;
}
...
static struct PyMethodDef rasterizer_methods[] = {
	...
	{"setFullScreen", (PyCFunction)gPySetFullScreen, METH_VARARGS, ""},
...
PyMODINIT_FUNC initRasterizerPythonBinding()
{
	...
	/* Set Fullscreen */
	KX_MACRO_addTypesToDict(d, KX_WINDOW, 0);
	KX_MACRO_addTypesToDict(d, KX_WINDOW_FULLSCREEN, 1);
	KX_MACRO_addTypesToDict(d, KX_WINDOW_BORDERLESS, 2);
...
std::string pathGamePythonConfig(char *saveName, char *extName)
{
	...
	/* replace extension */
	bool isRange = BLI_path_extension_check(path.c_str(), ".range");
	bool isRangeTilde = BLI_path_extension_check(path.c_str(), ".range~");
	if (isRange || isRangeTilde) {
		// remove .range extension.
		size_t extLen = isRangeTilde ? 7 : 6;
		path = path.substr(0, len - extLen);
		len = path.size();

		// remove filename and add saveName.
		if (saveName) {
			path = path.substr(0, len - strlen(BLI_path_basename(path.c_str())));
  			path += saveName;
		}
	}
	else {
