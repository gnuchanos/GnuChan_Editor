...
static bool getNavmeshNormal(dtNavMesh* navmesh, dtNavMeshQuery* navquery, const mt::vec3& pos, mt::vec3& normal)
{
    ...
    dtStatus status = navquery->findNearestPoly(spos.Data(), polyPickExt, &filter, &polyRef, nearestPt);
    if (dtStatusFailed(status) || polyRef == 0) {
        return false;
    }

    // Decode tile and poly index (using bit shift)
    int tileIndex = (int)(polyRef >> 16);
    int polyIndex = (int)(polyRef & 0xFFFF);
    ...
    for (int i = 0; i < pd->triCount; ++i) {
        ...
        float dist = barDistSqPointToTri(spos.Data(), v[0], v[1], v[2]);
        if (dist < distMin) {
            distMin = dist;
            idxMin = i;
        }
    ...
    for (int j = 0; j < 3; ++j) {
        const float* v;
        if (t[j] < p->vertCount) {
            v = &tile->verts[p->verts[t[j]] * 3];
        } else {
            v = &tile->detailVerts[(pd->vertBase + (t[j] - p->vertCount)) * 3];
        }
        float posFlipped[3] = { v[0], v[1], v[2] };
        flipAxes(posFlipped);
        tri[j] = mt::vec3(posFlipped);
    }
...
void KX_SteeringActuator::HandleActorFace(const mt::vec3& velocity)
{
    ...
    if (m_navmesh && m_normalUp) {
        ...
        if (navmesh && navquery) {
            ...
            if (getNavmeshNormal(navmesh, navquery, trpos, normal)) {
    ...
    switch (m_facingMode) {
        ...
        default:
            // if mode not provided, does it maintain mat identity ? Or does it do nothing ?
            break;
    }

    mat = mt::mat3(left, dir, up);

    // if the object has a parent, adjust the local orientation relative to the parent
