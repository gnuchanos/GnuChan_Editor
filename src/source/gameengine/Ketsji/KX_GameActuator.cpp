	...
	:SCA_IActuator(gameobj, KX_ACT_GAME)
{
	m_mode = mode;
	m_filename = filename;
 	m_savelocation = nullptr;
	if (strlen(savelocation.c_str()) != 0) {
		m_savelocation = new char[1024];
		strcpy(m_savelocation, savelocation.c_str());
	}
	m_extensionname = nullptr;
	if (strlen(extensionname.c_str()) != 0) {
		m_extensionname = new char[1024];
		strcpy(m_extensionname, extensionname.c_str());
	}
...
#ifdef WITH_PYTHON
			if (m_ketsjiengine) {
				saveGamePythonConfig(m_savelocation, m_extensionname, nullptr);
			}
			break;
#endif // WITH_PYTHON
		}
...
#ifdef WITH_PYTHON
			if (m_ketsjiengine) {
				loadGamePythonConfig(m_savelocation, m_extensionname);
			}
			break;
#endif // WITH_PYTHON
		}
...
PyAttributeDef KX_GameActuator::Attributes[] = {
    ...
    EXP_PYATTRIBUTE_STRING_RW("saveLocation", 0, 100, false, KX_GameActuator, m_savelocation),
    EXP_PYATTRIBUTE_STRING_RW("extensionName", 0, 100, false, KX_GameActuator, m_extensionname),
