...
{
    dtNavMesh* navmesh = navmeshobj->GetNavMesh();
    if (!navmesh) {
        return;
    }
    ...
    for (int i = 0; i < maxTiles; ++i) {
        ...
        for (int pi = 0; pi < nPolys; ++pi) {
            ...
            for (int vi = 0, vj = (int)poly->vertCount - 1; vi < (int)poly->vertCount; vj = vi++) {
                ...
                const float* vjPos = &tile->verts[poly->verts[vj] * 3];
                const float* viPos = &tile->verts[poly->verts[vi] * 3];

                KX_Obstacle* obstacle = CreateObstacle(navmeshobj);
                obstacle->m_type = KX_OBSTACLE_NAV_MESH;
                obstacle->m_shape = KX_OBSTACLE_SEGMENT;
                obstacle->m_pos = mt::vec3(vjPos[0], vjPos[2], vjPos[1]);
                obstacle->m_pos2 = mt::vec3(viPos[0], viPos[2], viPos[1]);
                obstacle->m_rad = 0;
            }
        }
    }
