...
static void rna_def_font(BlenderRNA *UNUSED(brna), StructRNA *srna)
{
	...
	RNA_def_property_ui_text(prop, "Use shadows", "Use shadows in text");
	RNA_def_property_update(prop, 0, "rna_Curve_update_data");

	prop = RNA_def_property(srna, "shadow_offset_x", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "shadow_offx");
	RNA_def_property_range(prop, -1000, 1000);
	RNA_def_property_ui_text(prop, "Shadow offset X", "");
	RNA_def_property_update(prop, 0, "rna_Curve_update_data");

	prop = RNA_def_property(srna, "shadow_offset_y", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "shadow_offy");
	RNA_def_property_range(prop, -1000, 1000);
	RNA_def_property_ui_text(prop, "Shadow offset Y", "");
	RNA_def_property_update(prop, 0, "rna_Curve_update_data");

	prop = RNA_def_property(srna, "shadow_col", PROP_FLOAT, PROP_COLOR);
	RNA_def_property_array(prop, 4);
	RNA_def_property_ui_text(prop, "Shadow Color", "Shadow color for text");
	RNA_def_property_update(prop, 0, "rna_Curve_update_data");
