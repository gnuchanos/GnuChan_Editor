...
static void rna_Lamp_buffer_csm_two_size_set(PointerRNA *ptr, int value)
{
	Lamp *la = (Lamp *)ptr->data;

	CLAMP(value, 128, 10240);
	la->bufsizetwo = value;
	la->bufsizetwo &= (~15); /* round to multiple of 16 */
}

static void rna_Lamp_buffer_csm_three_size_set(PointerRNA *ptr, int value)
{
	Lamp *la = (Lamp *)ptr->data;

	CLAMP(value, 128, 10240);
	la->bufsizethree = value;
	la->bufsizethree &= (~15); /* round to multiple of 16 */
}

static void rna_Lamp_csm_three_proportion_set(PointerRNA *ptr, float value)
{
	Lamp *la = (Lamp *)ptr->data;

	la->csm_proportion_three = value - la->csm_proportion_two;
...
static void rna_def_lamp_shadow(StructRNA *srna, int spot, int area)
{
	...
	RNA_def_property_boolean_sdna(prop, NULL, "mode", LA_CSM_SHADOW);
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	prop = RNA_def_property(srna, "use_csm_debug", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mode", LA_CSM_DEBUG_SHADOW);
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	prop = RNA_def_property(srna, "shadow_csm_two_buffer_size", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "bufsizetwo");
	RNA_def_property_range(prop, 128, 10240);
	RNA_def_property_ui_text(prop, "Cascasded Two Shadow Buffer Size",
	                         "Resolution of the shadow buffer, higher values give crisper shadows "
	                         "but use more memory");
	RNA_def_property_int_funcs(prop, NULL, "rna_Lamp_buffer_csm_two_size_set", NULL);
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_csm_three_buffer_size", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "bufsizethree");
	RNA_def_property_range(prop, 128, 10240);
	RNA_def_property_ui_text(prop, "Cascasded Two Shadow Buffer Size",
	                         "Resolution of the shadow buffer, higher values give crisper shadows "
	                         "but use more memory");
	RNA_def_property_int_funcs(prop, NULL, "rna_Lamp_buffer_csm_three_size_set", NULL);
	RNA_def_property_update(prop, 0, "rna_Lamp_update");
...
static void rna_def_sun_lamp(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_range(prop, 0.001, 10000.0, 2, 1);
	RNA_def_property_ui_text(prop, "Frustum Size", "Size of the frustum used for creating the shadow map");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	/* Cascaded params */
	prop = RNA_def_property(srna, "cascaded_proportion_two", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_proportion_two");
	RNA_def_property_range(prop, 0.001f, 0.5f);
	RNA_def_property_ui_text(prop, "Cascaded Proportion Two", "CSM Near Proportion");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	prop = RNA_def_property(srna, "shadow_buffer_soft_two", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "soft_two");
	RNA_def_property_range(prop, 0.1f, 100.0f);
	RNA_def_property_ui_text(prop, "Shadow Buffer Soft", "Size of shadow buffer sampling area");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_bias_two", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "bias_two");
	RNA_def_property_range(prop, 0.001f, 5.0f);
	RNA_def_property_ui_text(prop, "Shadow Buffer Bias Two", "Shadow buffer sampling bias");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "cascaded_proportion_three", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_proportion_three");
	RNA_def_property_range(prop, 0.001f, 0.5f);
	RNA_def_property_ui_text(prop, "Cascaded Proportion Three", "CSM Middle Proportion");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	prop = RNA_def_property(srna, "shadow_buffer_soft_three", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "soft_three");
	RNA_def_property_range(prop, 0.1f, 100.0f);
	RNA_def_property_ui_text(prop, "Shadow Buffer Soft", "Size of shadow buffer sampling area");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_bias_three", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "bias_three");
	RNA_def_property_range(prop, 0.001f, 5.0f);
	RNA_def_property_ui_text(prop, "Shadow Buffer Bias Three", "Shadow buffer sampling bias");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");
	
	prop = RNA_def_property(srna, "shadow_blend", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "spotblend");
	RNA_def_property_range(prop, 0.0f, 512.0f);
