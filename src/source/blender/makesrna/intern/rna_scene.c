...
static void rna_SCENEFXSettings_gbuff_fx_update(Main *UNUSED(bmain), Scene *scene, PointerRNA *UNUSED(ptr))
{
	...
	// SSAO GI need albedo buffer
	if (!gm->attachments[1]) {
		if ((scene->scenefx_settings.scenefx_flag & SCENE_FX_FLAG_SSAO_GI)) {
			RenderAttachment *attach = MEM_callocN(sizeof(RenderAttachment), "RenderAttachment");
			attach->type = GAME_ATTACHMENT_ALBEDO;
			attach->size = 3;
			attach->hdr = GAME_HDR_NONE;
			BLI_strncpy(attach->name, "Attachment", MAX_NAME);
			gm->attachments[1] = attach;
		}
	}
	else {
		gm->attachments[1]->type = GAME_ATTACHMENT_ALBEDO;
		gm->attachments[1]->hdr = GAME_HDR_NONE;
	}
...
static void rna_def_scene_game_data(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Borderless Window", "Launches the player in fullscreen mode without window borders or title bar");
	RNA_def_property_update(prop, NC_SCENE, NULL);
...
static void rna_def_scene_gpu_tonemap_fx(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Temperature", "Color Temperature");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}
...
static void rna_def_scene_gpu_ssao_fx(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "gi_irradiance", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Irradiance", "Global Illumination Irradiance");
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_ui_range(prop, 0.0f, 100.0f, 1, 3);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}

static void rna_def_scene_gpu_vignette_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "GPUVIGNETTESettings", NULL);
	RNA_def_struct_ui_text(srna, "GPU VIGNETTE", "Settings for GPU based vignette");
	RNA_def_struct_ui_icon(srna, ICON_RENDERLAYERS);

	prop = RNA_def_property(srna, "size", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Vignette Size", "Vignette Size");
	RNA_def_property_range(prop, 0.0f, 20.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "radius", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Vignette Radius", "Vignette Radius");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}
...
static void rna_def_scene_shaders_fx(BlenderRNA *brna)
{
	...
	rna_def_scene_gpu_vignette_fx(brna);
	...
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_gbuff_fx_update");
	...
	prop = RNA_def_property(srna, "use_ssao_gi", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_SSAO_GI);
	RNA_def_property_ui_text(prop, "Screen Space Irradiance", "Use SSI on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_gbuff_fx_update");

	prop = RNA_def_property(srna, "ssao_lod", PROP_ENUM, PROP_NONE);
	RNA_def_property_ui_text(prop, "SSAO Lod", "Resolution at which the SSAO will be rendered on the screen");
	RNA_def_property_enum_bitflag_sdna(prop, NULL, "ssao_lod");
	RNA_def_property_enum_items(prop, custom_offscreen_lod_items);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	prop = RNA_def_property(srna, "vignette", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_struct_type(prop, "GPUVIGNETTESettings");
	RNA_def_property_ui_text(prop, "Vignette settings", ""); 

	prop = RNA_def_property(srna, "use_vignette", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_VIGNETTE);
	RNA_def_property_ui_text(prop, "VIGNETTE", "Use vignette on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");

	prop = RNA_def_property(srna, "show_expanded_vignette", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_VIGNETTE);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_vignette", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_VIGNETTE);
	RNA_def_property_ui_text(prop, "Render Vignette", "Render Vignette in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}
...
void RNA_def_scene(BlenderRNA *brna)
{
	...
	func = RNA_def_function(srna, "range_statistics", "ED_info_stats_range_string");
	parm = RNA_def_string(func, "range_statistics", NULL, 0, "Range_Statistics", "");
	RNA_def_function_return(func, parm);
