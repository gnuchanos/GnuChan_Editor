...
const EnumPropertyItem rna_enum_image_generated_type_items[] = {
	...
	{IMA_GENTYPE_GRID_PLUS, "UV_GRID_PLUS", 0, "UV Grid+", "Generated grid optimized for testing UV mappings in game development"},
...
static void rna_def_image(BlenderRNA *brna)
{
	...
	RNA_def_property_boolean_sdna(prop, NULL, "flag", IMA_MIPMAP);
	RNA_def_property_ui_text(prop, "Use Mipmap", "Generate and use mipmaps for this image. Disable to keep the image sharp and pixelated");
	RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, "rna_Image_colormanage_update");

	prop = RNA_def_property(srna, "use_anisotropic", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", IMA_ANISOTROPIC_FILTER);
	RNA_def_property_ui_text(prop, "Use Anisotropic", "Improve texture clarity at oblique viewing angles, Disabling may result in blurrier textures when viewed from sharp angles");
	RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, "rna_Image_colormanage_update");
