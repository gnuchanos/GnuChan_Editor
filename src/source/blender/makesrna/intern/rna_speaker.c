...
static void rna_def_speaker(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem prop_type_items[] = {
		{SPEAKER_PLAY_END_SOUND, "PLAYEND", 0, "Play End", ""},
		{SPEAKER_LOOP_END_SOUND, "LOOPEND", 0, "Loop End", ""},
		{SPEAKER_LOOP_BIDIRECTIONAL_SOUND, "LOOPBIDIRECTIONAL", 0, "Loop Bidirectional", ""},
		{0, NULL, 0, NULL, NULL}
	};
	...
	prop = RNA_def_property(srna, "cache", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", SPK_SOUND_CACHE);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_ui_text(prop, "Cache Sound",
		"Saves the sound in RAM, this can greatly improve the speed for repetitive play calls, but "
		"it takes up a lot of RAM space. Consider using small files");
	RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_ID_SOUND);
	...
	prop = RNA_def_property(srna, "start_init", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", SPK_START_INIT);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_ui_text(prop, "Start Init", "The sound starts when the game starts");

	prop = RNA_def_property(srna, "destroy_after", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", SPK_DESTROY_AFTER);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_ui_text(prop, "Destroy After Play", "Remove the speaker from the scene after finishing the sound");

	prop = RNA_def_property(srna, "start_at", PROP_FLOAT, PROP_NONE);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_range(prop, 0.0f, 10000.0f);
	RNA_def_property_ui_text(prop, "Start At", "When should the sound start (in seconds)");

	prop = RNA_def_property(srna, "use_sound_3d", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", SPK_USE_3DSOUND);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
