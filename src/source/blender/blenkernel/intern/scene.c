...
void BKE_scene_fx_validate(Scene *scene)
{
	    ...
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_TONEMAP))
	{
		...
		fx_tonemap->saturation = 1.0f;
		fx_tonemap->temperature = 0.5f;
	}
	    ...
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_LIGHTSCATTER))
	{
		...
		fx_scatter->intensity = 0.5f;
		fx_scatter->threshold = 0.75f;
		fx_scatter->stepsize = 1.0f;
	    ...
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_SSAO))
	{
		...
		/* init settings */
		fx_settings.ssao_lod = 2;
		fx_ssao->factor = 2.0f;
		fx_ssao->distance_max = 0.5f;
		fx_ssao->attenuation = 1.0f;
		fx_ssao->samples = 16;

		fx_ssao->use_gi = 0;
		fx_ssao->gi_irradiance = 0.2f;
	}
	if ((fx_settings.vignette == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_VIGNETTE))
	{
		GPUVIGNETTESettings *fx_vignette;
		fx_vignette = fx_settings.vignette = MEM_callocN(sizeof(GPUVIGNETTESettings), __func__);

		/* init settings */
		fx_vignette->size = 10.0f;
		fx_vignette->radius = 0.5f;
	}
...
void BKE_scene_copy_data(Main *bmain, Scene *sce_dst, const Scene *sce_src, const int flag)
{
	...
	if (sce_src->scenefx_settings.vignette)
		sce_dst->scenefx_settings.vignette = MEM_dupallocN(sce_src->scenefx_settings.vignette);
...
void BKE_scene_free(Scene *sce)
{
	...
	if (sce->scenefx_settings.vignette)
		MEM_freeN(sce->scenefx_settings.vignette);
