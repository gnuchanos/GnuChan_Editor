...
static void image_buf_fill_new_checker_slice(unsigned char *rect,
                                              float *rect_float,
                                              int width, int height,
                                              int offset)
{
	...
	int subgridwidth = 64;
	int smallgridwidth = 32;
	int x, y, dark;

	unsigned char *rect_orig = rect;
	float *rect_float_orig = rect_float;

	float dark_linear_color = srgb_to_linearrgb(0.28f);
	float bright_linear_color = srgb_to_linearrgb(0.70f);
	float subgrid_linear_color = srgb_to_linearrgb(0.8f);
	float smallgrid_linear_color = srgb_to_linearrgb(0.4f);
	...
	if (rect_float || rect) {
		for (y = offset; y < height + offset; y++) {
			...
			for (x = 0; x < width; x++) {
				...
				bool draw_subgrid = (x % subgridwidth == 0) || (y % subgridwidth == 0);
				bool draw_smallgrid = (x % smallgridwidth == 0) || (y % smallgridwidth == 0);

				if (rect_float) {
					if (draw_subgrid) {
						rect_float[0] = rect_float[1] = rect_float[2] = subgrid_linear_color;
						rect_float[3] = 1.0f;
					}
					else if (draw_smallgrid) {
						rect_float[0] = rect_float[1] = rect_float[2] = smallgrid_linear_color;
						rect_float[3] = 1.0f;
					}
					else if (dark > 0) {
						rect_float[0] = rect_float[1] = rect_float[2] = dark_linear_color;
					...
					if (draw_subgrid) {
						rect[0] = rect[1] = rect[2] = 210;
						rect[3] = 255;
					}
					else if (draw_smallgrid) {
						rect[0] = rect[1] = rect[2] = 130;
						rect[3] = 255;
					}
					else if (dark > 0) {
						rect[0] = rect[1] = rect[2] = 75;
						rect[3] = 255;
					}
					else {
						rect[0] = rect[1] = rect[2] = 180;
						rect[3] = 255;
					}
					rect += 4;
				}
...
static void checker_board_resolution_text(unsigned char *rect,
                                          float *rect_float,
                                          int width,
                                          int height)
{
    ...
    int text_len = snprintf(text, sizeof(text), "%ix%i", width, height);
    ...
    /* Outline */
    /*BLF_buffer_col(mono, outline_color);
    BLF_position(mono, pen_x - 2, pen_y, 0.0f);
    BLF_draw_buffer(mono, text, text_len);
    BLF_position(mono, pen_x + 2, pen_y, 0.0f);
    BLF_draw_buffer(mono, text, text_len);
    BLF_position(mono, pen_x, pen_y - 2, 0.0f);
    BLF_draw_buffer(mono, text, text_len);
    BLF_position(mono, pen_x, pen_y + 2, 0.0f);
    BLF_draw_buffer(mono, text, text_len);*/
    ...
    BLF_position(mono, pen_x, pen_y, 0.0f);
    BLF_draw_buffer(mono, text, text_len);
                                             ...
                                             int num_scanlines)
{
	...
	if (!data->use_new) {
		...
		image_buf_fill_new_checker_slice(rect, rect_float, data->width, num_scanlines, start_scanline);
	}
}

void BKE_image_buf_fill_checker(unsigned char *rect,
                                float *rect_float,
                                int width, int height, const bool use_new)
{
	if (((size_t)width) * height < 64 * 64) {
		if (!use_new) {
			...
			image_buf_fill_new_checker_slice(rect, rect_float, width, height, 0);
		}
	}
	else {
		...
		data.rect_float = rect_float;
		data.width = width;
		data.use_new = use_new;
		IMB_processor_apply_threaded_scanlines(
		    height, image_buf_fill_checker_thread_do, &data);
	}

	if (use_new)
		checker_board_resolution_text(rect, rect_float, width, height);
}
