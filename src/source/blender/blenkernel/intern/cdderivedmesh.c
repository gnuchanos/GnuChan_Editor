	...
	if (setDrawOptions != NULL) {
		...
		for (a = 0; a < tot_active_mat; a++) {
			...
			if (do_draw) {
				...
				GPU_buffer_draw_elements(dm->drawObject->triangles, (use_points ? GL_POINTS : GL_TRIANGLES),
				                         dm->drawObject->materials[a].start, dm->drawObject->materials[a].totelements);
				if (matconv[a].numdata) {
...
static void cdDM_drawFacesGLSL(DerivedMesh *dm, DMSetMaterial setMaterial, bool use_points)
{
