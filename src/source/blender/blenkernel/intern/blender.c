...
void BKE_range_version_string(char *version_str, size_t maxncpy, short version, short subversion, short minsubversion, bool v_prefix, bool include_subversion)
{
	...
	if (include_subversion) {
		...
		// Release Rev Version.
		else if (minsubversion > 1000) {
			// Rev Versions.
			BLI_snprintf(version_str, maxncpy, (minsubversion == 1001) ? "%s%d.%d Rev1" : "%s%d.%d Rev2", prefix, version, subversion, minsubversion);
			
		}
		// Release Sub Versions.
		else if (minsubversion > 100) {
			// A and B Releases.
