...
static bool wm_file_write(bContext *C, const char *filepath, int fileflags, ReportList *reports)
{
	...
	if (BLI_exists(filepath) && !BLI_file_is_writable(filepath)) {
		BKE_reportf(reports, RPT_ERROR, "Cannot save range file, path '%s' is not writable", filepath);
		return ok;
	}
...
static bool protected_save_check(bContext* UNUSED(C), wmOperator* op)
{
	...
	/* first check if it has .range extension, if it has change it to .rasec */
	if (BLI_path_extension_check(filepath, ".range")) {
		BLI_path_extension_replace(filepath, sizeof(filepath), ".rasec");
		RNA_string_set(op->ptr, "filepath", filepath);
		return true;
	}
	else if (!BLI_path_extension_check(filepath, ".rasec")) {
		/* some users would prefer BLI_path_extension_replace(),
		 * we keep getting nitpicking bug reports about this - campbell */
		BLI_path_extension_ensure(filepath, FILE_MAX, ".rasec");
		RNA_string_set(op->ptr, "filepath", filepath);
		return true;
	}
	return false;
...
void WM_OT_save_as_mainfile_protected(wmOperatorType* ot) {
	...
	ot->name = "Save As RanGE File Protected";
...
void WM_OT_save_as_mainfile(wmOperatorType *ot)
{
	...
	WM_operator_properties_filesel(
	        ot, FILE_TYPE_FOLDER | FILE_TYPE_BLENDER, FILE_BLENDER, FILE_SAVE,
	        WM_FILESEL_FILEPATH, FILE_DEFAULTDISPLAY, FILE_SORT_ALPHA);
	RNA_def_boolean(ot->srna, "compress", false, "Compress",
