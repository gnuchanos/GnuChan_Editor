...
{
  const bool previewVer = RANGE_MINSUBVERSION < 100;
  const bool isPostRelease = RANGE_MINSUBVERSION > 100;
  const bool isPostRevRelease = RANGE_MINSUBVERSION > 1000;

  if (!isPostRelease) {
	  BLI_snprintf(ver,
				max_length,
				previewVer ? "Range Engine %d.%d Preview %d" : "Range Engine %d.%d %s",
				RANGE_VERSION,
				RANGE_SUBVERSION,
				previewVer ? RANGE_MINSUBVERSION : "Release"
               );
  }
  else if (isPostRevRelease) {
	// Rev Releases.
    BLI_snprintf(ver,
                 max_length,
                 (RANGE_MINSUBVERSION == 1001) ? "Range Engine %d.%d Rev1 Release" : "Range Engine %d.%d Rev2 Release",
                 RANGE_VERSION,
		         RANGE_SUBVERSION,
		         RANGE_MINSUBVERSION);
  }
    ...
    BLI_snprintf(ver,
                 max_length,
                 (RANGE_MINSUBVERSION == 101) ? "Range Engine %d.%da %s" : "Range Engine %d.%db %s",
                 RANGE_VERSION,
                 RANGE_SUBVERSION,
                 previewVer ? RANGE_MINSUBVERSION : "Release");
  }
...
static uiBlock *wm_block_create_about(bContext *C, ARegion *region, void *UNUSED(arg))
{
  ...
  const uchar *range_logo_data = (const uchar *)datatoc_range_logo_png;
  size_t range_logo_data_size = datatoc_range_logo_png_size;
  ImBuf *ibuf = IMB_ibImageFromMemory(
      range_logo_data, range_logo_data_size, IB_rect, NULL, "range_logo");
  ...
  uiItemL_ex(col, N_("Range Game Engine"), ICON_NONE, true, false);
...
{
  ot->name = "About Range Game Engine";
  ot->idname = "WM_OT_splash_about";
  ot->description = "Open a window with information about Range Game Engine";
