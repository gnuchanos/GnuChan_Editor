...
uniform vec2 toneColorParams; // Saturation, Temperature
...
void main() {
	...
	if (tonemapParams.x == 0.0) {
		vec3 rgb = color.rgb;
		rgb *= max(tonemapParams.y * 12.70, 0.001);
		...
		rgb = pow(tonemapped, vec3(6.0 / max(tonemapParams.z, 0.001)));
	...
	gl_FragColor.rgb = setTemperature(gl_FragColor.rgb, toneColorParams.y); // [0.0, 1.0]
	gl_FragColor.rgb = setSaturation(gl_FragColor.rgb, toneColorParams.x); // [0.0, 2.0]
}
