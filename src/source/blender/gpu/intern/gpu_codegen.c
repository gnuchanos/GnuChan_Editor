...
extern char datatoc_gpu_shader_fragment_usercode_lib_glsl[];
...
static char *code_generate_fragment(ListBase *nodes, char *usercode, const GPUMatType type, GPUNodeLink *outputs[8])
{
	...
	if (usercode) {
		if (strncmp(usercode, "#define RangeLib", strlen("#define RangeLib")) == 0) {
			BLI_dynstr_append(ds, datatoc_gpu_shader_fragment_usercode_lib_glsl); // UserCode libs
		}
...
static char *code_generate_vertex(ListBase *nodes, char *usercode, const GPUMatType type, bool use_instancing)
{
	...
	if (usercode) {
		for (node = nodes->first; node; node = node->next) {
			for (input = node->inputs.first; input; input = input->next) {
				if (input->source == GPU_SOURCE_ATTRIB && input->attribfirst) {
					if (input->attribtype == CD_MTFACE && input->type == 2 && strcmp(input->attribname, "UV2") == 0)
						BLI_dynstr_appendf(ds, "vec2 UV2 = att%d;\n", input->attribid);
					else if (input->attribtype == CD_MTFACE && input->type == 2 && strcmp(input->attribname, "UV3") == 0)
						BLI_dynstr_appendf(ds, "vec2 UV3 = att%d;\n", input->attribid);
					else if (input->attribtype == CD_MTFACE && input->type == 2)
