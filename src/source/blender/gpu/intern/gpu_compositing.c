...
typedef struct {
  int color_uniform;
  int vignetteParams_uniform;
} GPUVIGNETTEShaderInterface;

typedef struct {
  int color_uniform;
  int tonemapParams_uniform;
  int toneColorParams_uniform;
        ...
        const GPUFXSettings *fx_settings)
{
	...
	if (!(scene->scenefx_settings.editor_render_flag & SCENE_FX_FLAG_SSAO_GI)) {
		scenefx_flag &= ~SCENE_FX_FLAG_SSAO_GI;
	}
	if (!(scene->scenefx_settings.editor_render_flag & SCENE_FX_FLAG_FXAA)) {
		scenefx_flag &= ~SCENE_FX_FLAG_FXAA;
	}
	if (!scenefx_settings->vignette || !(scene->scenefx_settings.editor_render_flag & SCENE_FX_FLAG_VIGNETTE)) {
		scenefx_flag &= ~SCENE_FX_FLAG_VIGNETTE;
	}
	...
	if (scenefx_flag & SCENE_FX_FLAG_VIGNETTE)
		num_passes++;
        ...
        struct Scene *scene, struct GPUOffScreen *ofs)
{
	...
	if (fx->sce_effects & SCENE_FX_FLAG_TONEMAP) {
		...
		if (tonemap_shader) {
			...
			float paramsCol[2] = {fx_tonemap->saturation, fx_tonemap->temperature};
			...
			GPU_shader_uniform_vector(tonemap_shader, interface->toneColorParams_uniform, 2, 1, paramsCol);
	...
	/* vignette pass */
	if (fx->sce_effects & SCENE_FX_FLAG_VIGNETTE) {
		GPUShader *vignette_shader;
		vignette_shader = GPU_shader_get_builtin_fx_shader(GPU_SHADER_FX_VIGNETTE, is_persp);
		if (vignette_shader) {
			const GPUVIGNETTESettings *fx_vignette = scenefx.vignette;
			float params[2] = {fx_vignette->size, fx_vignette->radius};

			GPUVIGNETTEShaderInterface *interface = GPU_shader_get_interface(vignette_shader);

			GPU_shader_bind(vignette_shader);

			GPU_texture_bind(src, numslots++);
			GPU_shader_uniform_texture(vignette_shader, interface->color_uniform, src);

			GPU_shader_uniform_vector(vignette_shader, interface->vignetteParams_uniform, 2, 1, params);
...
void GPU_fx_shader_init_interface(struct GPUShader *shader, GPUFXShaderEffect effect)
{
	...
	switch (effect) {
		...
		case GPU_SHADER_FX_TONEMAP:
		{
			...
			interface->toneColorParams_uniform = GPU_shader_get_uniform(shader, "toneColorParams");
		...
		case GPU_SHADER_FX_VIGNETTE:
		{
			GPUVIGNETTEShaderInterface *interface = MEM_mallocN(sizeof(GPUVIGNETTEShaderInterface), "GPUVIGNETTEShaderInterface");

			interface->color_uniform = GPU_shader_get_uniform(shader, "colorbuffer");
			interface->vignetteParams_uniform = GPU_shader_get_uniform(shader, "vignetteParams");
