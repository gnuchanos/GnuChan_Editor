...
#undef SCENE_STATS_FMT_INT
	...
	s_statistics = stats->infostr;
	ofs = 0;

	//ofs += BLI_snprintf(s + ofs, MAX_INFO_LEN - ofs, "Range Engine %s | ", range_versionstr);

	if (scene->obedit) {
		if (BKE_keyblock_from_object(scene->obedit))
			ofs += BLI_strncpy_rlen(s_statistics + ofs, IFACE_("(Key) "), MAX_INFO_LEN - ofs);

		if (scene->obedit->type == OB_MESH) {
			ofs += BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs,
			...
			ofs += BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs, IFACE_("Verts:%s/%s | Bones:%s/%s"), stats_fmt.totvertsel,
			                    stats_fmt.totvert, stats_fmt.totbonesel, stats_fmt.totbone);
		}
		else {
			ofs += BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs, IFACE_("Verts:%s/%s"), stats_fmt.totvertsel,
			                    stats_fmt.totvert);
		}

		ofs += BLI_strncpy_rlen(s_statistics + ofs, memstr, MAX_INFO_LEN - ofs);
		ofs += BLI_strncpy_rlen(s_statistics + ofs, gpumemstr, MAX_INFO_LEN - ofs);
	}
	else if (ob && (ob->mode & OB_MODE_POSE)) {
		ofs += BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs, IFACE_("Bones:%s/%s %s%s"),
		                    stats_fmt.totbonesel, stats_fmt.totbone, memstr, gpumemstr);
	}
	else if (stats_is_object_dynamic_topology_sculpt(ob)) {
		ofs += BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs, IFACE_("Verts:%s | Tris:%s%s"), stats_fmt.totvert,
	...
	if (ob)
		BLI_snprintf(s_statistics + ofs, MAX_INFO_LEN - ofs, " | %s", ob->id.name + 2);
...
const char *ED_info_stats_range_string(Scene *scene)
{
	if (!scene->stats)
		stats_update(scene);

	BLI_snprintf(scene->stats->rangestr, 64, "Range Engine %s", range_versionstr);

	return scene->stats->rangestr;
}
