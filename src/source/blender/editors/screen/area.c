...
static void draw_azone_plus(float x1, float y1, float x2, float y2, AZEdge edge)
{
	...
	const float points[6][2] = { {0, -hl},        {l, hl},         {l - s, hl + s},
								 {0, s + s - hl}, {s - l, hl + s}, {-l, hl} };
	const float center[2] = { (x1 + x2) / 2, (y1 + y2) / 2 };

	const int axis = (edge == AE_RIGHT_TO_TOPLEFT) ? 0 : 1;
	
	glBegin(GL_TRIANGLE_FAN);
	for (int i = 0; i < 6; i++) {
		// Left
		if (axis == 0)
			glVertex2f(center[0] + points[i][1] * -1 * size, center[1] + points[i][0] * size);
		// Right
		else
			glVertex2f(center[0] + points[i][1] * 1 * size, center[1] + points[i][0] * size);
	};
...
static void region_draw_azone_tab_plus(AZone *az)
{
	...
	const float x1 = (az->edge == AE_RIGHT_TO_TOPLEFT) ? az->x1 - 5 : az->x1 + 5;
	const float x2 = (az->edge == AE_RIGHT_TO_TOPLEFT) ? az->x2 - 5 : az->x2;
	UI_draw_roundbox(x1, (float)az->y1, x2, (float)az->y2, 6.0f);

	glEnable(GL_BLEND);

	glColor4f(0.8f, 0.8f, 0.8f, 0.6f);
	draw_azone_plus(x1, (float)az->y1, (float)az->x2, (float)az->y2, az->edge);
