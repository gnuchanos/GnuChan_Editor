...
static void draw_view_axis(RegionView3D *rv3d, rcti *rect)
{
	...
	int bright = 10; //-40 * (10 - U.rvibright); /* axis alpha offset (rvibright has range 0-10) */
	 ...
	 */
	float startx = rect->xmax - 15.0f - (k + UI_UNIT_X);
	float starty = rect->ymax - 5.0f - (k + UI_UNIT_Y);

	//startx += rect->xmin; fix, now that he's on the right we don't care about the left anymore.
	starty += rect->ymin;
	...
	// Circle Background
	unsigned char col[4] = {0.0f, 0.0f, 0.0f, 25.0f};
	glColor4ubv(col);

	// Background Outline
	// circ(startx, starty, 41);

	glBegin(GL_TRIANGLE_FAN);
	for (int j = 0; j < 32; j++) {
		float angle = 2.0f * 3.1415f * j / 32;
		float x = startx + 41 * cosf(angle);
		float y = starty + 41 * sinf(angle);
		glVertex2f(x, y);
	}
	glEnd();
	...
	const unsigned int cube_edges[12][2] = {
	...
	glBegin(GL_LINES);
	for (int i = 0; i < 12; i++) {
		int start_idx = cube_edges[i][0];
		int end_idx = cube_edges[i][1];

		float start[3] = { cube_vertices[start_idx][0], cube_vertices[start_idx][1], cube_vertices[start_idx][2] };
		float end[3] = { cube_vertices[end_idx][0], cube_vertices[end_idx][1], cube_vertices[end_idx][2] };

		mul_qt_v3(rv3d->viewquat, start);
		mul_qt_v3(rv3d->viewquat, end);

		float sx = startx + start[0] * k;
		float sy = starty + start[1] * k;
		float ex = startx + end[0] * k;
		float ey = starty + end[1] * k;
	...
	for (axis_i = 0; axis_i < 3; axis_i++) {
		...
		if (fabsf(dx) > toll || fabsf(dy) > toll) {
			...
			/* Adding the circle */
			float centerX = startx + dx;
			float centerY = starty + dy + ydisp;

			float radius = 8.0f;

			unsigned char col[4];
			UI_GetThemeColor4ubv(TH_AXIS_X + i, col);
			col[3] = 100;
			glColor4ubv(col);

			/* Adding the inverse circle */
			float reverseX = startx - dx;
			float reverseY = starty - dy + ydisp;

			// Circle Fill
			glBegin(GL_POLYGON);
			for (int j = 0; j < segments; j++) {
				float angle = 2.0f * 3.1415f * j / segments;
				float x = reverseX + radius * cosf(angle);
				float y = reverseY + radius * sinf(angle);
				glVertex2f(x, y);
			}
			glEnd();
			...
			BLF_draw_default_ascii(centerX - 2.5f, centerY - 2.8f, 0.0f, axis_text, 1);
	...
	for (axis_i = 0; axis_i < 3; axis_i++) {
		...
		if (fabsf(dx) > toll || fabsf(dy) > toll) {
			...
			glColor3f(0.2f, 0.2f, 0.2f);
			BLF_draw_default_ascii(centerX - 2.8f, centerY - 2.8f, 0.0f, axis_text, 1);
...
static void draw_viewport_name(Scene *scene, ARegion *ar, View3D *v3d, rcti *rect)
{
	RegionView3D *rv3d = ar->regiondata;
	const char *name = view3d_get_name(v3d, rv3d);
	const char *info_stats = ED_info_stats_string(scene);
	...
	float o[2] = {U.widget_unit + rect->xmin - 2.0f, rect->ymax - U.widget_unit - 15.5f};

	float statslen = 1;
	if (strlen(info_stats) <= 0) statslen = 1;
	else { statslen = strlen(info_stats); }

	// Statistics Background

	float width = 4.5f * statslen;
	float height = 25.0f;
	...
	glBegin(GL_QUADS);
	glVertex2f(o[0], o[1] - height);
	glVertex2f(o[0] + width, o[1] - height);
	glVertex2f(o[0] + width, o[1] + height);
	glVertex2f(o[0], o[1] + height);
	glEnd();
...
#else
	BLF_draw_default_ascii(U.widget_unit + rect->xmin,  rect->ymax - U.widget_unit - 12.0f, 0.0f, name, sizeof(tmpstr));
	BLF_draw_default_ascii(U.widget_unit + rect->xmin,  rect->ymax - U.widget_unit, 0.0f, info_stats, sizeof(tmpstr));
#endif
}
...
static void draw_selected_name(Scene *scene, Object *ob, const rcti *rect)
{
	...
	BLF_draw_default(offset - 5.0f, rect->ymax - 56.0f, 0.0f, info, sizeof(info));
}
...
static void gpu_update_lamps_shadows_world(Main *bmain, Scene *scene, View3D *v3d)
{
	...
	for (shadow = shadows.first; shadow; shadow = shadow->next) {
		...
		ARegion ar = {NULL};
		RegionView3D rv3d = {{{0}}};
		const bool vsm = GPU_lamp_shadow_buffer_type(shadow->lamp) == LA_SHADMAP_VARIANCE;
		const bool use_cascaded = GPU_lamp_has_csm(shadow->lamp);
		...
		GPU_lamp_calc_winmat(shadow->lamp, use_cascaded ? 1 : 0);
		GPU_lamp_shadow_buffer_bind(shadow->lamp, 0, viewmat, &winsize, winmat);
		...
		if (!vsm && use_cascaded) {
			for (int i = 1; i < 3; i++) {
				// we need to calculate winmat individually for each cascaded. Recalculate in GPU_material_update_lamps too.
				GPU_lamp_calc_winmat(shadow->lamp, (i == 1) ? 2 : 0);
				GPU_lamp_shadow_buffer_bind(shadow->lamp, i, viewmat, &winsize, winmat);
	...
	if (world) {
		...
		GPU_mist_update_values(world->mistype, world->miststa, world->mistdist, world->mistheight, world->mistdensity, world->misi, &world->fogr);
        ...
        GPUOffScreen *ofs)
{
	...
	if (v3d->fx_settings.fx_flag && fx) {
		...
		GPUVIGNETTESettings *vignette = NULL;
		...
		if (v3d->drawtype < OB_SOLID) {
			...
			vignette = scene->scenefx_settings.vignette;
			scene->scenefx_settings.vignette = NULL;
		}
		...
		if (vignette)
			scene->scenefx_settings.vignette = vignette;
	}
...
#else
	BLF_draw_default_ascii(rect->xmin + U.widget_unit + 80.0f,  rect->ymax - U.widget_unit - 36.0f, 0.0f, printable, sizeof(printable));
#endif
}
                                       ...
                                       const char *grid_unit, bool render_border)
{
	...
	if ((v3d->flag2 & V3D_RENDER_OVERRIDE) == 0) {
		...
		if (U.uiflag & USER_SHOW_VIEWPORTNAME) {
			ED_scene_draw_fps(scene, &rect);
			draw_viewport_name(scene, ar, v3d, &rect);
		}
		...
		if (grid_unit) { /* draw below the viewport name */
			...
			BLF_draw_default_ascii(rect.xmin + U.widget_unit,
			                       rect.ymax - (USER_SHOW_VIEWPORTNAME ? 2 * U.widget_unit : U.widget_unit) - 4.0f, 0.0f,
