...
static void ui_block_region_refresh(const bContext *C, ARegion *region)
{
	...
	if (region->do_draw & RGN_DRAW_REFRESH_UI) {
		ScrArea *handle_ctx_area;
		ARegion *handle_ctx_region;

		region->do_draw &= ~RGN_DRAW_REFRESH_UI;
...
static void ui_block_region_draw(const bContext *C, ARegion *region)
{
	...
	/*
	 * LATE :
	 * Hack To Make Popover Somewhat More Stable
	 * By Make It Refresh After Drawing .
	 */
	ui_block_region_refresh((bContext *)C, region);
}
