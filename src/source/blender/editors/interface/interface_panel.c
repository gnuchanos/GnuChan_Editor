...
static int panel_aligned(ScrArea *sa, ARegion *ar)
{
	if (sa->spacetype == SPACE_BUTS && ar->regiontype == RGN_TYPE_WINDOW) {
		...
		return BUT_VERTICAL;
	else if (ELEM(ar->regiontype, RGN_TYPE_UI, RGN_TYPE_TOOLS, RGN_TYPE_TOOL_PROPS, RGN_TYPE_NAV_BAR))
		return BUT_VERTICAL;
...
void UI_draw_icon_tri(float x, float y, char dir)
{
	...
	float f3 = 0.02 * U.widget_unit;
	float f5 = 0.15 * U.widget_unit;
	float f7 = 0.12 * U.widget_unit;

	if (dir == 'h') {
		UI_draw_anti_tria(x - f3, y - f5, x - f3, y + f5, x + f7, y, color, 3.0f);
	}
	else if (dir == 't') {
		UI_draw_anti_tria(x - f5, y - f7, x + f5, y - f7, x, y + f3, color, 3.0f);
	}
	else { /* 'v' = vertical, down */
		UI_draw_anti_tria(x - f5, y + f3, x + f5, y + f3, x, y - f7, color, 3.0f);
	}
}
...
static void ui_draw_aligned_panel_header(uiStyle *style, uiBlock *block, const rcti *rect, char dir)
{
	...
	if (dir == 'h') {
		...
		UI_fontstyle_draw(fontstyle, &hrect, activename);
	}
	else {
		...
		UI_fontstyle_draw_rotated(fontstyle, &hrect, activename);
	}
}
...
void ui_draw_aligned_panel(uiStyle *style, uiBlock *block, const rcti *rect, const bool show_pin)
{
	...
	/* an open panel */
		...
		if (is_subpanel) {
			glEnable(GL_BLEND);
			UI_ThemeColor4(TH_PANEL_SUB_BACK);
