...
static bool text_run_shader_script_poll(bContext *C)
{
	return (CTX_data_active_object(C) != NULL);
}

static int text_run_shader_script_exec(bContext *C, wmOperator *op)
{
...
void TEXT_OT_run_shader_script(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Run Shader Script";
	ot->idname = "TEXT_OT_run_shader_script";
	ot->description = "Recompiles the shader material of the selected object";

	/* api callbacks */
	ot->poll = text_run_shader_script_poll;
	ot->exec = text_run_shader_script_exec;
