...
static int game_animation_events_copy_exec(bContext *C, wmOperator *UNUSED(op))
{
	Object *ob = ED_object_active_context(C);
	...
	{
		if (ob != ob_iter) {
			/* first: free all animation events */
			BKE_animation_event_free_list(&ob_iter->animevents);
			
			/* now copy it */
			BKE_animation_event_copy_list(&ob_iter->animevents, &ob->animevents);
		}
	}
	CTX_DATA_END;
...
void OBJECT_OT_game_animation_events_copy(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Copy Animation Events to Selected";
	ot->description = "Copy Animation Events to other selected objects";
	ot->idname = "OBJECT_OT_game_animation_events_copy";

	/* api callbacks */
	ot->exec = game_animation_events_copy_exec;
	ot->poll = ED_operator_object_active_editable;
