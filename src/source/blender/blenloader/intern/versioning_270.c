...
/* NOTE: this version patch is intended for versions < 2.52.2, but was initially introduced in 2.27
 * already. But in 2.79 another case generating non-unique names was discovered (see T55668,
 * involving Meta strips)... */
...
void blo_do_versions_270(FileData *fd, Library *UNUSED(lib), Main *bmain)
{
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 270, 4)) {
    /* ui_previews were not handled correctly when copying areas, leading to corrupted files (see
     * T39847). This will always reset situation to a valid state.
     */
    bScreen *sc;
...
#undef LA_YF_PHOTON
...

    {
      Object *ob;
      for (ob = bmain->object.first; ob; ob = ob->id.next) {
        if (ob->body_type == OB_BODY_TYPE_CHARACTER && (ob->gameflag & OB_BOUNDS) &&
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 276, 5)) {
    ...
    /* Important to clear all non-persistent flags from older versions here, otherwise they could
     * collide with any new persistent flag we may add in the future. */
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 279, 4)) {
    ...
    if (!DNA_struct_elem_find(fd->filesdna, "Brush", "float", "falloff_angle")) {
      for (Brush *br = bmain->brush.first; br; br = br->id.next) {
        ...
        br->flag &= ~(BRUSH_FLAG_DEPRECATED_1 | BRUSH_FLAG_DEPRECATED_2 | BRUSH_FLAG_DEPRECATED_3 |
                      BRUSH_FLAG_DEPRECATED_4 | BRUSH_FRONTFACE_FALLOFF);
      }
