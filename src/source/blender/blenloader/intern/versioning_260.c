...
static void do_versions_nodetree_convert_angle(bNodeTree *ntree)
{
  ...
  for (node = ntree->nodes.first; node; node = node->next) {
    if (node->type == CMP_NODE_ROTATE) {
      ...
      /* XXX DNA char to float conversion seems to map the char value into the [0.0f, 1.0f]
       * range... */
...
static void do_versions_nodetree_multi_file_output_format_2_62_1(Scene *sce, bNodeTree *ntree)
{
  ...
  for (node = ntree->nodes.first; node; node = node->next) {
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      ...
      /* ugly, need to remove the old inputs list to avoid bad pointer checks when adding new
       * sockets. sock->storage is expected to contain path info in
...
#define DefNode(Category, ID, DefFunc, EnumName, StructName, UIName, UIDesc) \
  ...
  /* XXX hack, group types share a single static integer identifier, but are registered as separate
   * types */
  if (type == NODE_GROUP) {
    switch (treetype) {
      case NTREE_COMPOSIT:
  ...
  if (bmain->versionfile < 263 || (bmain->versionfile == 263 && bmain->subversionfile < 19)) {
    ...
    for (ima = bmain->image.first; ima; ima = ima->id.next) {
      if (ima->source == IMA_SRC_VIEWER) {
        ...
        /* if colormanagement not used, set image's color space to raw, so no sRGB->linear
         ...
         * cases when color management was used for only some of scenes, but such a setup is crazy
         * anyway and think it's fair enough to break compatibility in that cases
         */
